...
PolyLine::PolyLine(QList<QPointF> pointList)
{
...
PolyLine::PolyLine(QList<QPointF> pointList, QList<qreal> pressureList, double tol)
{
    ...
    PolyLine::simplify(tol, pointList, 0, n-1, markList);
...
QDomElement PolyLine::createDomElement(QDomDocument& doc)
{
...
void PolyLine::loadDomElement(QDomElement element)
{
...
void PolyLine::setOrigin(const QPointF& point)
{
    origin = point;
}

void PolyLine::setOrigin(const QPointF& point, const qreal& pressureValue, const bool& trueOrFalse)
{
    origin = point;
...
//void PolyLine::setC1(int i, const QPointF& point)
//{
//    if ( i >= 0 || i < c1.size() )
//    {
//        c1[i] = point;
...
//    else
//    {
//        qDebug() << "PolyLine::setC1! index out of bounds:" << i;
//    }
//}

//void PolyLine::setC2(int i, const QPointF& point)
//{
//    if ( i >= 0 || i < c2.size() )
//    {
//        c2[i] = point;
...
//    else
//    {
//        qDebug() << "PolyLine::setC2! index out of bounds:" << i;
//    }
//}

void PolyLine::setVertex(int i, const QPointF& point)
{
    if (i==-1) { origin = point; }
    else
    {
        ...
        {
            qDebug() << "PolyLine::setVertex! index out of bounds:" << i;
        }
    }
}

void PolyLine::setLastVertex(const QPointF& point)
{
    if (vertex.size()>0)
    {
    ...
    {
        qDebug() << "PolyLine::setLastVertex! curve has less than 2 vertices";
    }
}


void PolyLine::setWidth(qreal desiredWidth)
{
    width = desiredWidth;
}

void PolyLine::setFeather(qreal desiredFeather)
{
    feather = desiredFeather;
}

void PolyLine::setVariableWidth(bool YesOrNo)
{
    variableWidth = YesOrNo;
}

void PolyLine::setInvisibility(bool YesOrNo)
{
    invisible = YesOrNo;
}

void PolyLine::setSelected(int i, bool YesOrNo)
{
    selected[i+1] = YesOrNo;
}

PolyLine PolyLine::transformed(QTransform transformation)
{
    PolyLine transformedCurve = *this; // copy the curve
...
void PolyLine::transform(QTransform transformation)
{
...
//void PolyLine::appendCubic(const QPointF& c1Point, const QPointF& c2Point, const QPointF& vertexPoint, qreal pressureValue)
//{
//    //c1.append(c1Point);
//    //c2.append(c2Point);
...
void PolyLine::addPoint(int position, const QPointF point)
{
    ...
    {
        qDebug() << "Error PolyLine::addPoint(int, QPointF)";
    }
}
...
//void PolyLine::drawPath(QPainter& painter, Object* object, QTransform transformation, bool simplified, bool showThinLines )
//{
...
//        }
//        QPainterPath path = myCurve.getPath();
...
//QPainterPath PolyLine::getStrokedPath()
//{
//    return getStrokedPath( width );
//}

//QPainterPath PolyLine::getStrokedPath(qreal width)
//{
//    return getStrokedPath(width, true);
//}

//QPainterPath PolyLine::getStrokedPath(qreal width, bool usePressure)
//{
//    QPainterPath path;
...
void PolyLine::simplify(double tol, QList<QPointF>& inputList, int j, int k, QList<bool>& markList)
{
...
qreal PolyLine::eLength(const QPointF point)    // calculates the Euclidean Length (of a point seen as a vector)
{
...
qreal PolyLine::mLength(const QPointF point)   // calculates the Manhattan Length (of a point seen as a vector)
{
...
void PolyLine::normalise(QPointF& point)
{
...
//qreal PolyLine::findDistance(PolyLine curve, int i, QPointF P, QPointF& nearestPoint, qreal& t)   //finds the distance between a cubic section and a point
//{
...
//QPointF PolyLine::getPointOnCubic(int i, qreal t)
//{
...
bool PolyLine::intersects(QPointF point, qreal distance)
{
    bool result = false;
...
bool PolyLine::intersects(QRectF rectangle)
{
    bool result = false;
    if (drawPath().controlPointRect().intersects(rectangle))
    {
        for(int i=0; i<vertex.size(); i++)
        {
...
bool PolyLine::findIntersection(PolyLine curve1, int i1, PolyLine curve2, int i2, QList<Intersection>& intersections)   //finds the intersection between two cubic sections
{
    bool result = false;
