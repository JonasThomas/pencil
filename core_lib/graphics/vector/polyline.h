...
public:
    PolyLine();
    PolyLine(QList<QPointF> pointList);
    PolyLine(QList<QPointF> pointList, QList<qreal> pressureList, double tol);
...
//    QRectF getBoundingRect();
    ...
    static qreal findDistance(PolyLine curve, int i, QPointF P, QPointF& nearestPoint, qreal& t); //finds the distance between a cubic section and a point
    static bool findIntersection(PolyLine curve1, int i1, PolyLine curve2, int i2, QList<Intersection>& intersections); //finds the intersection between two cubic sections
